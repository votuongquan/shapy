...
if __name__ == '__main__':
    resnet = resnet50(pretrained=True,
                      replace_stride_with_dilation=None)
    dilated_resnet = resnet50(
        pretrained=True,
        replace_stride_with_dilation=[False, True, True])
    device = torch.device('cuda')
    resnet = resnet.to(device=device)
    dilated_resnet = dilated_resnet.to(device=device)

    B, C, H, W = 32, 3, 256, 256

    images = torch.zeros([B, C, H, W], dtype=torch.float32, device=device)

    output = resnet(images)['layer4']
    logger.info(f'No dilation: {output.shape}')

    output = dilated_resnet(images)['layer4']
    logger.info(f'With dilation: {output.shape}')
